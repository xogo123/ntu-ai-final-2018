...
class MCTS(AIAlgo):
    class mcts_node(object):
        def __init__(self, visit_n=1, state=None, init_score=None, parent=None, player=None, parent_action=None):
            """ set init visit_n=1 to avoid division by zero 
            set init total_score=0.5 to encourage discover new node
            """
            self.total_score = init_score
            ...
            self.child_nodes = []
    def __init__(self, turns=12, iter_n=999, init_score=1):
        """ 
        turns: depth of Tree
    ...
    def selection(self, node):
        ...
        while node.child_nodes:
            ...
            if scores != []:
                ...
                mean_score = 0
                std_score = 0.1
            total_visit_n = sum([child_node.visit_n-1 for child_node in node.child_nodes if child_node.visit_n > 1])
            scores_normal = [max(np.sqrt((np.log2(total_visit_n)/max(child_node.visit_n-1,1))*2) + (((child_node.total_score / max(child_node.visit_n-1,1) - mean_score) / std_score) + 2) / 4, 0.001) for child_node in node.child_nodes]
            node = random.choices(node.child_nodes, scores_normal)[0]
            # node = node.child_nodes[np.argmax(np.array(scores_normal))]
        return node, level_i
    def expansion(self, node):
        node.child_nodes = [ self.mcts_node(state=node.state.next_turn(a), parent=node, init_score=self.init_score, player=node.state.next_turn(a).now_player, parent_action=a) for a in node.state.actions() ]    
    ...
    def backpropagation(self, state, node):
        while node.parent:
            if node.visit_n == 2:
                node.total_score -= self.init_score
            node.total_score += node.parent.player.h(state) - self.dict_player_init_h[node.parent.player.idx]#/ self.dict_player_init_h[node.parent.player.idx]
            node.visit_n += 1
            node = node.parent
        return None
    def get_action(self, state):
